...
def set_expert_selection(draft, value):
    for name, module in draft.named_modules():
        if isinstance(module, LlamaMLP):
            module.expert_selection = value

def check_expert_selection(draft):
    return any(isinstance(module, LlamaMLP) and module.expert_selection for name, module in draft.named_modules())